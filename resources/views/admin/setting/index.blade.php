    ...
    <form method="POST" action="{{ route('admin.setting.update') }}" enctype="multipart/form-data"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="site_description" rows="3" class="w-full p-3 rounded-xl border border-pink-100">{{ $setting->site_description }}</textarea>
            </div>

            <div>
                <label class="text-sm text-gray-600">Logo</label>
                <input type="file" name="site_logo" class="w-full p-3 rounded-xl border border-pink-100 bg-white">
            ...
            <div>
                ...
                <input type="file" name="site_favicon" class="w-full p-3 rounded-xl border border-pink-100 bg-white">
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="site_meta_description" rows="3" class="w-full p-3 rounded-xl border border-pink-100">{{ $setting->site_meta_description }}</textarea>
            </div>

            <div class="md:col-span-2">
                <label class="text-sm text-gray-600">Google Analytics Script</label>
                <textarea name="site_google_analytics" rows="4" class="w-full p-3 rounded-xl border border-pink-100">{{ $setting->site_google_analytics }}</textarea>
            </div>

            <div class="md:col-span-2">
                <label class="text-sm text-gray-600">Meta Pixel Script</label>
                <textarea name="site_meta_pixel" rows="4" class="w-full p-3 rounded-xl border border-pink-100">{{ $setting->site_meta_pixel }}</textarea>
            </div>
            <div class="md:col-span-2">
                <h3 class="text-lg font-bold text-slate-800 border-t border-pink-100 pt-6">
                    Theme Color
                </h3>
            </div>
            ...
            <div>
                ...
                <input type="color" name="theme_primary" value="{{ $setting->theme_primary ?? '#ec4899' }}"
                    class="w-full h-12 rounded-xl border border-pink-100 bg-white">
            </div>
